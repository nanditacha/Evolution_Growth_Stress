...
trl=950;%trehalose at which no lag

%growth rate
%growthr=g0*(1-tr/trg)^2
trg=1000; %trehalose for no growth
g0=40;
...
for param=1:Nparam %different values of chosen parameter
    r0=4*10^4+2*(param)*10^4;
    disp('resource value is')
    disp(r0)
    for rep=1:Nreplicates %each replicate for given parameter value
        ...
        for N=1:Ncycles %different cycles of FT        
            ...
            % 
            % if size(tr1,2)>nsamples
            %     temp=randsample(tr1,nsamples);
            %     tempsize=nsamples;
            % else
            %     temp=tr1;
            %     tempsize=size(tr1,2);
            % end
            % 
            %tr1large(rep,N,1:tempsize)=temp;
            ...
            % if size(tr2,2)>nsamples
            %     temp=randsample(trsurv,nsamples);
            %     tempsize=nsamples;
            % else
            %     temp=tr2;
            %     tempsize=size(tr2,2);
            ...
            if mod(N,10)==0
                save("trsim_iterate_compare_resource_350cycles.mat");
            end
        end
    end
end
